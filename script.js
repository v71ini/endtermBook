function LoadBK(filename, displayName) {
    let cbook = "";
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            cbook = xhr.responseText;

            getDocStats(cbook);

            cbook = cbook.replace(/(?:\r\n|\r|\n)/g, '<br>');

            document.getElementById("fileContent").innerHTML = cbook;
...
function performMark() {
    ...
    var Bookcont = display.innerHTML;

    NewCont = Bookcont.replace(re, replaceText);
